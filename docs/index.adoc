...
If the client is hosted on a different domain than the GraphQL service the browser will not allow the client to talk to the backend unless proper CORS headers are set. There is an outcommented example in the GraphQL service source code (src/main/resources/services/graphql/graphql.js).
